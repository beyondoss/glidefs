...
async fn main() -> anyhow::Result<()> {
    ...
    use glidefs::oci::boot_capture_served::{capture_boot_blocks_served, BootProfileOptions};
    use glidefs::oci::sandbox::{select_sandbox, Sandbox};
    ...
    // ---- capture the boot set through the HARDENED serving profiler ----
    // Delegates to the production `capture_boot_blocks_served`, so the research
    // binary runs the (trusted) image entrypoint through the same isolation
    // sandbox (namespaces + cgroup limits + hard timeout) as bless/profile, rather
    // than the old chroot + host-bind path. `GLIDEFS_PROFILE_TIMEOUT` (default 15s)
    // and `GLIDEFS_PROFILE_RUNS` (default 1) tune the run.
    let pcfg = settings.profile.clone().unwrap_or_default();
    let sandbox: Arc<dyn Sandbox> = Arc::from(select_sandbox(&pcfg.sandbox_config(None, true))?);
    let timeout = std::env::var("GLIDEFS_PROFILE_TIMEOUT")
        .ok()
        .and_then(|s| s.parse().ok())
        .map_or(std::time::Duration::from_secs(15), std::time::Duration::from_secs);
    let runs = std::env::var("GLIDEFS_PROFILE_RUNS").ok().and_then(|s| s.parse().ok()).unwrap_or(1);
    eprintln!("[profile] serving {name} ({fs_type}) — running {run_cmd:?} under sandbox");
    ...
    .await
    .ok_or_else(|| anyhow::anyhow!("workload read nothing — no boot set captured"))?;
