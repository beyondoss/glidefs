...
async fn flush_and_sync(
    ...
    // Hold flush_lock for the entire flush_packs + sync_manifest
    // cycle so the handoff freeze fence (`router.freeze_all` ->
    // `wait_for_inflight_flush`) can serialize against us.
