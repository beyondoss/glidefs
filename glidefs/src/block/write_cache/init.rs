...
impl WriteCache<Initializing> {
    ...
    pub fn open(config: WriteCacheConfig) -> Result<WriteCache<Recovering>, CacheError> {
        ...
        // session. Rewriting ensures a clean WAL for future appends.
        {
            ...
            // Atomic rewrite: truncate + write valid entries only.
            // Include entries with seq <= persisted_max_seq (filtered by replay)
            // since they were already applied to state_map by load_metadata.
            // The WAL file must contain ALL entries that haven't been
            // checkpointed yet — partial entries use original sequences.
