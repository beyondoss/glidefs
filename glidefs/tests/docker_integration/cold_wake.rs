...
transport_test! {
    async fn test_cold_wake_from_different_node(transport) {
        let ctx = TestContext::new().await;
        let db_path = "wake-test";

        // --- Server A: write data, drain, shutdown ---
        let server_a = TestServer::start(Arc::clone(&ctx.object_store), db_path, transport).await;
        ...
        let server_b = TestServer::start(Arc::clone(&ctx.object_store), db_path, transport).await;
...
transport_test! {
    async fn test_cold_wake_multiple_blocks(transport) {
        let ctx = TestContext::new().await;
