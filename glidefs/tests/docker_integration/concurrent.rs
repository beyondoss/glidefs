...
#[tokio::test]
async fn test_concurrent_clients_same_export() {
    let ctx = TestContext::new().await;
    let server = TestServer::start(Arc::clone(&ctx.object_store), "concurrent-clients").await;
    ...
    for client_idx in 0..4u64 {
        ...
        let handle = tokio::spawn(async move {
            let mut client = NbdClient::connect(addr, "vol1").await.unwrap();
...
#[tokio::test]
async fn test_drain_during_active_writes() {
    let ctx = TestContext::new().await;
