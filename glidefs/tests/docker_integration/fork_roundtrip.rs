...
transport_test! {
    async fn test_fork_flush_cold_wake(transport) {
        let ctx = TestContext::new().await;

        // --- Phase 1: create parent, write 4 blocks, snapshot ---
        let server = TestServer::start(Arc::clone(&ctx.object_store), "fork-cold-wake", transport).await;
        ...
        let server2 = TestServer::start(Arc::clone(&ctx.object_store), "fork-cold-wake", transport).await;
...
transport_test! {
    async fn test_fork_independent_of_parent(transport) {
        let ctx = TestContext::new().await;

        // --- Phase 1: create parent, write block 0, snapshot ---
        let server = TestServer::start(Arc::clone(&ctx.object_store), "fork-isolation", transport).await;
        server.create_export("parent", 0.01).await;
        ...
        // Same db_path so packs and manifests are visible in S3.
        // Use restore_forked_export since child's data lives under parent's S3 prefix.
        let server2 = TestServer::start(Arc::clone(&ctx.object_store), "fork-isolation", transport).await;
        server2.restore_forked_export("child", "parent", 0.01).await;
