...
transport_test! {
    async fn test_live_migration_readonly_fork_promote(transport) {
        let ctx = TestContext::new().await;

        // --- Server A: create export, write 4 blocks, snapshot ---
        let server_a = TestServer::start(Arc::clone(&ctx.object_store), "mig-a", transport).await;
        ...
        let server_b = TestServer::start(Arc::clone(&ctx.object_store), "mig-a", transport).await;
...
transport_test! {
    async fn test_migration_survives_cold_wake(transport) {
        let ctx = TestContext::new().await;

        // --- Server A: create export, write 2 blocks, snapshot, shutdown ---
        let server_a = TestServer::start(Arc::clone(&ctx.object_store), "cold-mig-a", transport).await;
        server_a.create_export("vm1", 0.01).await;
        ...
        let server_b = TestServer::start(Arc::clone(&ctx.object_store), "cold-mig-a", transport).await;
        server_b.fork_export_readonly("vm1", "vm1", 0.01).await;
        server_b.promote_export("vm1").await;
        ...
        let server_c = TestServer::start(Arc::clone(&ctx.object_store), "cold-mig-a", transport).await;
