...
proptest! {
    ...
    fn prop_write_read_roundtrip(
        ...
        rt.block_on(async {
            ...
            let (cache2, content_store2, pack_index_cache2, volume_manifest2, clean_cache2, metrics2) =
                super::create_cold_reader(&temp_dir2, "proptest", Arc::clone(&s3) as Arc<dyn object_store::ObjectStore>).await;

            let s3_data = cache2
                .read(offset, BLOCK_SIZE, clean_cache2.as_ref(), &pack_index_cache2, &volume_manifest2, &content_store2, &metrics2)
                .await
                .unwrap();
    ...
    fn prop_unwritten_blocks_zero(
        ...
        rt.block_on(async {
            ...
            let data = cache
                .read(offset, BLOCK_SIZE, clean_cache.as_ref(), &pack_index_cache, &volume_manifest, &content_store, &metrics)
                .await
                .unwrap();
